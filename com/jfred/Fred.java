// @(#)Fred.java 99/10/08 1.35 Robby Glen Garner, Paco Xander Nathan
// Copyright (c) 1997-99. Released under the GNU Public License
//
// this class implements the JFRED chat bot
...
public class Fred extends Object {
  ...
  //public Fred (File f, Grammar grammar, boolean isVerbose) {
  ...
  public Fred (Grammar grammar, boolean isVerbose, String sRulez) {
    RuleSetName = sRulez;
    this.grammar = grammar;
    this.isVerbose = isVerbose;
    setupDefaults();
...
//  public Fred (String RuleSetName, Grammar grammar, boolean isVerbose) {
//    this.grammar = grammar;
//    this.isVerbose = isVerbose;
//    setupDefaults();
...
//  }
  ...
  protected void setupDefaults () {
    ...
    //config.setDefault("rulestack.max", "50");
  }
  ...
  DataInputStream in = null;
	...
	// to the current rule
	  ...
	  if (r instanceof ResponseRule) {
		...
		System.err.println("Fred: fuzzy set probability not understood - " + prob);
		System.err.println(e.getMessage());
		e.printStackTrace();
	      }
	    }
      ...
      in.close();
      System.out.println(" " + (vActs.size() + vResp.size() + vRegx.size() + vFuzz.size()) + " rules");

      return true;
    }
    catch (IOException e) {
      System.err.println("Fred: I/O error - " + e.getMessage());
      e.printStackTrace();
  ...
  protected boolean testProperty (String s, Rule r) {
    if (matcher.contains(s, patProp)) {
      ...
      if (debug) {
	System.out.println("Fred: prop name - " + name + ", value - " + value);
      }
	...
	if (debug) {
	  System.out.println("Fred: adding URL " + value + " to rule " + r.getName());
	}
	...
	if (debug) {
	  System.out.println("Fred: adding response " + value + " to rule " + r.getName());
	}
